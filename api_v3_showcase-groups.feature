...
Feature: Testing of /api/v3/showcase-groups response
  ...
  @Web @/api/v3/showcase-groups
  Scenario: Testing status for showcase-groups for postpaid contract when web device give correct "limit" and incorrect "page"
    Given same contract as for previous scenario
    When "web" device has connection to service
    And this device sends requests to receive showcase-groups with "limit" equal "1" and "page" equal "-1"
    Then should be received "200" response status
    And should be received "error" response with correct structure

  @Web @/api/v3/showcase-groups
  Scenario: Testing status for showcase-groups for postpaid contract when web device give correct "page" and incorrect "limit"
    Given same contract as for previous scenario
    When "web" device has connection to service
    And this device sends requests to receive showcase-groups with "limit" equal "-1" and "page" equal "1"
    Then should be received "200" response status
